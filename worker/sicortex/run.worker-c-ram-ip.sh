...
#source /home/iraicu/.bashrc
 ...
 if [ -z "$3" ]; then 
              ...
              echo "usage: $0 128.135.125.17 56000 56001"
              exit
          fi


                           
export HOME_PATH=/home/iraicu/falkon/worker
...
#export IDLE_TIME=$2
            ...
            #mkdir -p ${HOME}/.falkon/logs
            #export LOG_FILE=${HOME}/.falkon/logs/worker.${MACH_ID}.${RANDOM}.txt
            export LOG_FILE=/dev/null
            /bin/echo "${MACH_ID}: writing logs to sicortex:${LOG_FILE}..."
...
if [ "${EXIT_CODE}" -ne "0" ]; then
    ...
    if [ ! -f "${HOME_PATH}/src-c/BGexec" ]; then
    ...
    cp ${HOME_PATH}/src-c/BGexec ${RAM_PATH}/${DIR_RAM}/

    EXIT_CODE=$? 

    if [ "${EXIT_CODE}" -ne "0" ]; then
        echo "Error in cp ${HOME_PATH}/src-c/BGexec ${RAM_PATH}/${DIR_RAM}/... exit code ${EXIT_CODE}"
        exit ${EXIT_CODE}
    fi
...
export FALKON_SERVICE_IP=$1


            /bin/echo "${MACH_ID}: starting worker and connecting to ${FALKON_SERVICE_IP} on ports $2 $3..."
...
#for ((b=0; b < 100 ; b++))
#do
            ${RAM_PATH}/${DIR_RAM}/BGexec ${FALKON_SERVICE_IP} $2 $3 -perf >> "${LOG_FILE}" 2>&1
            #${RAM_PATH}/${DIR_RAM}/BGexec ${FALKON_SERVICE_IP} $2 $3 -debug
	    #${RAM_PATH}/${DIR_RAM}/BGexec ${FALKON_SERVICE_IP} $2 $3 -debug >> "${LOG_FILE}" 2>&1
#${RAM_PATH}/${DIR_RAM}/BGexec_${PLATFORM} ${FALKON_SERVICE_IP} $5 $6 -debug	
#/bin/sleep 1
#done           
