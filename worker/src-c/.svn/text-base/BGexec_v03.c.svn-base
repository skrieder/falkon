...
void child_sig_handler (int x) {
    ...
    signal(SIGCLD, child_sig_handler);
    pid = wait(&status);
    kill(pid, SIGCLD);
}
...
recover:
  ...
  while (1)
  {
	...
	parseExec(msg, task);
	exitCode = run_task(task, ssfd, sfd, msg);
    }
    else{
	free(task);
	free(msg);
...
int run_task(Task *task, int ssfd, int sfd, char *msg)
{
  int exitCode;
  ...
  if((pid = fork())<0){
      ...
      execl("/bin/sh", "sh", "-c", task->command, (char *)0);
      _exit(127);
  }
  else{
