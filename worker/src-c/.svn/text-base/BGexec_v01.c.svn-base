...
int main(int argc, char**argv)
{
    ...
    */
	...
	//char buffer[10240];
	    ...
	    //strcat(msg, buffer);
	    numBytes = recv(sfd, msg, taskSize,  MSG_WAITALL);
            if (numBytes<0)
            {
...
int con_server(int port, char* argv1)
{
    ...
    while ( connect( sfd, (struct sockaddr *) &serv_addr, sizeof(serv_addr)) < 0)
    {
...
int sendResults(char* str, int exitCode, char** argv, int ssfd, int sfd)
{
    ...
    //int exitCode = 0;
    char *exitC;
    sprintf(exitC, "%d", exitCode);
    char* taskID=strcat(str,"#");
    if (exitCode==0)
        taskID=strcat(taskID, "0");
    else
        taskID=strcat(taskID, exitC);
    if (flag==1)
