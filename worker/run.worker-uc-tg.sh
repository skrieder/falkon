...
if [ "$?" -ne "0" ]; then
  echo "LAN GPFS not mounted... exit code $?" >> ${LOG_FILE}
    echo "${MACH_ID}: LAN GPFS not mounted... exit code $?" >> ${HOME}/falkon-provisioner-log.txt
  exit $?
fi
