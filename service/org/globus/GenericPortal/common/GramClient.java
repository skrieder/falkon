 ...
 * java -Daxis.ClientConfigFile=${GLOBUS_LOCATION}/client-config.wsdd -DGLOBUS_LOCATION=${GLOBUS_LOCATION} gram/GramClient
	...
	implements GramJobListener	 
{
	...
	throws Exception
	{
		...
		printMessage("Submission ID: " + submissionID);
                ...
                System.out.println("Add Listener for GRAM job...");
		if (!batch) {
			job.addListener(this);
		}
                System.out.println("Add Listener for GRAM job: SUCCESS!");
		...
		while (!submitted) {
			...
			try {
                                ...
                                System.out.println("Job submission try # " + tries + " ...");
				job.submit(factoryEndpoint, batch, this.limitedDelegation,
						submissionID);
                                System.out.println("Job submission try # " + tries + " : SUCCESS!");

				submitted = true;
			} catch (Exception e) {
                            System.out.println("Exception while submitting the job request, try # " + tries + ": " + e);
                            logger.debug("Exception while submitting the job request, try # " + tries + ": ", e);
                            if (this.wrg != null) 
                            {
                                logger.debug("De-allocating " + this.workersToAllocate + " workers due to GRAM job failure...");
                                this.wrg.decAllocatedWorkers(this.workersToAllocate);
                            }
	 ...
	 */
        ...
        public WorkerRunGram wrg = null;
        public int workersToAllocate = 0;

	public void main_run(String contact, String rslFileName, String GRAM_factory_type, WorkerRunGram wrg, int workersToAllocate)
	{
            this.wrg = wrg;
            this.workersToAllocate = workersToAllocate;
            //read properties
            /*
            ...
            //props.put("X509_USER_PROXY", "/tmp/x509up_u2640");
            //props.put("X509_USER_CERT", "/home/iraicu/.globus/usercert.pem");
            //props.put("X509_USER_KEY", "/home/iraicu/.globus/userkey.pem");
    ...
    Here is an example of the contents of a properties file:
      ...
      */
		...
		// Factory type: Fork, Condor, PBS, LSF
            ...
            if (GRAM_factory_type.contentEquals(new StringBuffer("PBS")))
            {
                factoryType = ManagedJobFactoryConstants.FACTORY_TYPE.PBS;
            }
            else if (GRAM_factory_type.contentEquals(new StringBuffer("LSF")))
            {
                factoryType = ManagedJobFactoryConstants.FACTORY_TYPE.LSF;
            }
            else if (GRAM_factory_type.contentEquals(new StringBuffer("CONDOR")))
            {
                factoryType = ManagedJobFactoryConstants.FACTORY_TYPE.CONDOR;
            }
            else if (GRAM_factory_type.contentEquals(new StringBuffer("FORK")))
            {
                factoryType = ManagedJobFactoryConstants.FACTORY_TYPE.FORK;
            }
            else
            {
                //default PBS is already set
...
*/
        ...
        public static void writeRSL(String rslFile, String executable, int maxWallTime, String hostType, int hostCount, int idleTime, String project, int MAX_NUM_WORKERS_PER_HOST, String FALKON_HOME)
        {
            //if (hostType.contentEquals(new StringBuffer("")))
            if (hostType.equals("any"))
            {
                writeRSL_basic(rslFile,executable ,maxWallTime ,hostCount ,idleTime,project, MAX_NUM_WORKERS_PER_HOST, FALKON_HOME );


            }
            else
            {
                writeRSL_extensions(rslFile,executable ,maxWallTime ,hostType ,hostCount ,idleTime, project, MAX_NUM_WORKERS_PER_HOST, FALKON_HOME);

            }

        }

        public static void writeRSL_extensions(String rslFile, String executable, int maxWallTime, String hostType, int hostCount, int idleTime, String project, int MAX_NUM_WORKERS_PER_HOST, String FALKON_HOME)
        {
            int cpuCount = hostCount;
            //int processCount = hostCount*10;
            int processCount = hostCount*MAX_NUM_WORKERS_PER_HOST;


            String dirPath = null;
            
            int safetyMargin = 1000*60;

            int wallTime = (int) Math.max((maxWallTime*60*1000 - safetyMargin), safetyMargin);

           try {
               dirPath = (new File(executable)).getParent();
                BufferedWriter out = new BufferedWriter(new FileWriter(rslFile));
                out.write("<job>\n");
                out.write("    <executable>" + executable + "</executable>\n");
                if (dirPath != null) out.write("    <directory>" + dirPath + "</directory>\n");
                out.write("    <argument>" + wallTime + "</argument>\n");
                //out.write("    <argument>0</argument>\n");
                out.write("    <argument>" + idleTime + "</argument>\n");
                //out.write("    <argument>" + MAX_NUM_WORKERS_PER_HOST +"</argument>\n");
                out.write("    <argument>" + 1 +"</argument>\n");

                if (FALKON_HOME != null) 
                {

                out.write("    <environment>\n");
                out.write("         <name>FALKON_HOME</name>\n");
                out.write("         <value>" + FALKON_HOME + "</value>\n");
                out.write("    </environment>\n");
                }
                ...
                if (project != null)
                {
                    ...
                    out.write("    <project>" + project + "</project>\n");
                }
                

                out.write("    <maxWallTime>" + maxWallTime + "</maxWallTime>\n");
                out.write("    <extensions>\n");
                out.write("         <resourceAllocationGroup>\n");
                if (!hostType.startsWith("any"))
                    out.write("            <hostType>" + hostType + "</hostType>\n");
                else
                {
                    System.out.println("hostType = " + hostType);
                }
                out.write("            <hostCount>" + hostCount + "</hostCount>\n");
                out.write("            <cpuCount>" + cpuCount + "</cpuCount>\n");
                out.write("            <processCount>" + processCount + "</processCount>\n");
                out.write("         </resourceAllocationGroup>\n");
                out.write("     </extensions>\n");
                out.write("</job>\n");
                out.close();
            ...
            {
                System.out.println("Error in writing RSL file '" + rslFile + "': " + e);
            }

        }

        public static void writeRSL_basic(String rslFile, String executable, int maxWallTime, int hostCount, int idleTime, String project, int MAX_NUM_WORKERS_PER_HOST, String FALKON_HOME)
        {
            int cpuCount = hostCount;
            //int processCount = hostCount*10;
            int processCount = hostCount*MAX_NUM_WORKERS_PER_HOST;
           ...
           try {
               dirPath = (new File(executable)).getParent();
                ...
                out.write("<job>\n");
                out.write("    <executable>" + executable + "</executable>\n");
                //out.write("    <count>" + processCount + "</count>\n");
        ...
        public static void main(String args[])
        {
            ...
            if (args.length == 5)
            {
                contact = args[0];
                executable = args[1];
                maxWallTime = (new Integer(args[2])).intValue();
                hostType = args[3];
                hostCount = (new Integer(args[4])).intValue();
            ...
            {
                System.out.println("incorect # of args: java org/globus/GenericPortal/common/GramClient <contact> <executable> <maxWallTime> <hostType> <hostCount>");
                System.exit(0);
            }
            ...
            gramClient.writeRSL(rslFile,executable ,maxWallTime ,hostType ,hostCount, 600000, "default", 1, null );


            System.out.println("####################### WorkerRunThread started....");
            while(true)
            {
                long start = System.currentTimeMillis();
                System.out.println("####################### WorkerRunThread starting job submission test....");


                gramClient.main_run(contact, rslFile, "PBS", null, hostCount);

                System.out.println("####################### WorkerRunThread finished starting job submission test.... sleeping for " + sleepTime + " ms");

                long end = System.currentTimeMillis();
                long elapsedTimeMillis = end-start;

                long curSleepTime = sleepTime - elapsedTimeMillis;
                ...
                try
                {
                    ...
                    System.out.println("Sleeping for " + curSleepTime + " ms until the next job submission...");
                ...
                System.out.println("####################### WorkerRunThread ended job submission test....");
