...
public class Notification implements Serializable
{
    ...
    public Notification(int timeout, boolean DEBUG)
    {
        ...
        //old code to find a incrementing port
        /*
        ...
        */
        ...
        //new code to find a random port
        int max_num_errors = port_max-port;
        int num_errors = 0;
        while (num_errors<max_num_errors && socket_bind == false)
        {
            try
            {
                recvPort = (int)Math.floor(rand.nextDouble()*(port_max-port)+port);
                //socket = new DatagramSocket(recvPort);
                socket = new ServerSocket(recvPort);
            ...
            {
                //if(DEBUG) System.out.println("error: Notification(int timeout): socket = new DatagramSocket(recvPort); " + e.getMessage());
                if (DEBUG) System.out.println("warning, setting up server socket on port " + recvPort + " failed: " + e.getMessage());
                //recvPort++;
                num_errors++;
            }
...
/*
  ...
  */
    ...
    //public boolean send(String dest, String key)
    {
        ...
        //System.out.println("Notification: send() : " + dest + " " + key);
