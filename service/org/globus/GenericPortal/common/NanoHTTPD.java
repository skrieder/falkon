 ...
 */
public class NanoHTTPD extends Thread
{
    // ==================================================
    // API parts
    // ==================================================
    ...
    {
        if (this.DEBUG) System.out.println( method + " '" + uri + "' " );

        Enumeration e = header.propertyNames();
        while ( e.hasMoreElements())
        {
            String value = (String)e.nextElement();
            if (this.DEBUG) System.out.println( "  HDR: '" + value + "' = '" +
        ...
        {
            String value = (String)e.nextElement();
            if (this.DEBUG) System.out.println( "  PRM: '" + value + "' = '" +
    ...
    public NanoHTTPD( String rootDir, int port , boolean DEBUG ) throws IOException
    {

        this.DEBUG = DEBUG;
        this.rootDir = rootDir;
        this.myFileDir = new File(rootDir);
        myTcpPort = port;
                               /*
        ServerSocket ss0 = null;

        while (ss0 == null && myTcpPort < 60000)
        {

            try
            {

                ss0 = new ServerSocket( myTcpPort++ );
            }
            catch (Exception e)
            {
        ...
        if (ss0 == null)
        {
            throw new IOException("error, could not bind to any port");
        }
        else
        {
            System.out.println("Will start HTTP Server on port " + myTcpPort);
            ss0.close();
        }
        */
    ...
    public void run()
    {
        if (this.DEBUG) System.out.println( "NanoHTTPD 1.1 (C) 2001,2005-2007 Jarno Elonen... modified by Ioan Raicu 2008\n" +
                                            "(Command line options: [rootDir] [port])\n" );
        if (this.DEBUG) System.out.println( "Now serving files in port " + this.myTcpPort + " from " + this.rootDir + "...");
    ...
    public static void main( String[] args )
    {
        ...
        boolean DEBUG = true;
        System.out.println( "NanoHTTPD 1.1 (C) 2001,2005-2007 Jarno Elonen... modified by Ioan Raicu 2008\n" +
                            "(Command line options: [rootDir] [port])\n" );
...
if ( args.length > 1 &&
     args[1].toLowerCase().endsWith( "licence" ))
        ...
        */
        ...
        try
        {
            nh = new NanoHTTPD( rootDir, port, DEBUG );
        }
        catch ( IOException ioe )
        {
        ...
        nh.myFileDir = new File(rootDir);

        if (DEBUG) System.out.println( "Now serving files in port " + port + " from " + rootDir);
