...
END_NUM=32768
DATA_SIZE=1B
EXP_NAME=data_gpfs_cache_${DATA_SIZE}_${END_NUM}

echo "generating data of size ${DATA_SIZE}..."
for ((a=${START_NUM}; a <= ${END_NUM} ; a++))  # Double parentheses, and "LIMIT" with no "$".
do
  echo "/bin/cp file_${DATA_SIZE}_${a} /dev/null %%% file_${DATA_SIZE}_${a} %%% $GPFS_PATH/$DATA_SIZE/file_${a} %%% null %%% null %%%" >> $EXP_NAME 
