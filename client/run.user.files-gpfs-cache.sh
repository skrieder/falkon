...
#MAX_EXECUTORS=16
#MAX_NUM_TASKS=( 4096	4096	4096	4096	4096	2048	1024	128 )
#MAX_EXECUTORS=32
#MAX_NUM_TASKS=( 4096	4096	4096	4096	4096	2048	1024	256 )
#MAX_EXECUTORS=64
#MAX_NUM_TASKS=( 4096	4096	4096	4096	4096	2048	1024	512 )
...
echo "Doing test ${EXP_DESC[j]} for ${MAX_NUM_TASKS[i]} tasks with data size ${DATA_SIZE[i]} and 100% data locality on ${MAX_EXECUTORS} executors with no security..."
echo "Doing test ${EXP_DESC[j]} for ${MAX_NUM_TASKS[i]} tasks with data size ${DATA_SIZE[i]} and 100% data locality on ${MAX_EXECUTORS} executors with no security..." >> ${CLIENT_LOG} 2>&1
./run.user.auto.sh ${FALKON_SERVICE_URL} ${FALKON_SERVICE_PORT} workloads/cp/${EXP_DESC[j]}_${DATA_SIZE[i]} ${MAX_NUM_TASKS[i]} ${MAX_NUM_THREADS} etc/client-security-config.xml ${DATA_SIZE[i]} ${MAX_EXECUTORS} ${EXP_DESC[j]} 8 >> ${CLIENT_LOG} 2>&1
done                           # A construct borrowed from 'ksh93'.
done                           # A construct borrowed from 'ksh93'.
echo; echo
