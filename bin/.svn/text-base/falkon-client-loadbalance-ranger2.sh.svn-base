#!/bin/bash
 ...
 if [ -z "$5" ]; then 
              ...
              echo "usage: $0 null $FALKON_HOME/workloads/sleep/sleep_1x10 10 0001 1"
              exit 1
          fi
...
if [ ! -d "/tmp/${USER}/falkon" ]; then
    ...
    dd bs=128k if=${FALKON_HOME}/falkon_ranger.tgz of=/tmp/${USER}/falkon_ranger.tgz 

    EXIT_CODE=$? 
    if [ "${EXIT_CODE}" -ne "0" ]; then
        echo "Error in executing command 'dd bs=128k if=${FALKON_HOME}/falkon_ranger.tgz of=/tmp/${USER}/falkon_ranger.tgz'... exit code ${EXIT_CODE}" >> ${ERROR_LOG}
        DATE=`date`
        echo "${DATE} : falkon-client-loadbalance-ranger2 : failed : Error in executing command 'dd bs=128k if=${FALKON_HOME}/falkon_ranger.tgz of=/tmp/${USER}/falkon_ranger.tgz'... exit code ${EXIT_CODE}" >> ${LOG_FILE_DEBUG}
        exit ${EXIT_CODE}
    fi
...
tar fxz falkon_ranger.tgz
    ...
    if [ "${EXIT_CODE}" -ne "0" ]; then
        ...
        DATE=`date`
        echo "${DATE} : falkon-client-loadbalance-ranger2 : failed : Error in executing command 'tar fxz falkon_ranger.tgz'... exit code ${EXIT_CODE}" >> ${LOG_FILE_DEBUG}
        exit ${EXIT_CODE}
    fi
...
java -Xms1536M -Xmx1536M -Xss128K -Djava.security.egd=file:///dev/urandom -DFALKON_LOGS=${LOG_DIR} -classpath $CLASSPATH:${FALKON_CLIENT_HOME} org/globus/GenericPortal/clients/GPService_instance/UserRun -serviceURIfile ${SERVICE_LIST_FILE} -MAX_EXECS_PER_WS ${EXECS_PER_WS} -CLIENT_DESC ${SECURITY_DESCRIPTION_FILE} -job_description ${EXEC_FILE} -num_execs ${NUM_EXECS} -num_submit_threads ${NUM_THREADS} -monitor ${MONITOR_DELAY} -max_concurent_tasks ${MAX_CONCURENT_TASKS} -max_submit_throughput ${MAX_SUBMIT_THROUGHPUT} -taskPerfLog ${LOG_DIR}/client_taskPerf.txt -taskDescriptionLog ${LOG_DIR}/client_taskDescription.txt -maxWallTimeMS ${MAX_WALL_TIME_MS}
