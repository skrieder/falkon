...
DATE=`date`
echo $DATE : falkon-ion-start-blocking-zeptocn-ram: started >> ${LOG_FILE_DEBUG}
...
DATE=`date`
echo $DATE : falkon-ion-start-blocking-zeptocn-ram: data copied over >> ${LOG_FILE_DEBUG}
cd /tmp/${USER}
...
DATE=`date`
echo $DATE : falkon-ion-start-blocking-zeptocn-ram: untared >> ${LOG_FILE_DEBUG}
cd $1
...
LOG_DIR=${FALKON_LOGS}/service/
    ...
    falkon-tcpcore-start.sh ${IP} ${FALKON_PORT} >> /tmp/falkon-tcpcore-start.log
