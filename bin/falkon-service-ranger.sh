...
#echo "==========================================================================================================="
DATE=`date +%s`      
echo "$DATE: starting falkon-service-ranger.sh..."
...
DATE=`date +%s`      
#echo "$DATE: Setting log directory..." >> ${LOG_FILE}
echo "$DATE: Setting log directory..."


#EXP_START=`date +%Y.%m.%d_%H.%M.%S`
#LOG_DIR=${FALKON_LOGS}/service/${EXP_START}_${MACH_ID}
LOG_DIR=${FALKON_LOGS}/service/${MACH_ID}
#assumes it is pre-created
#mkdir -p ${LOG_DIR} 
...
#pwd >> ${LOG_FILE} 2>&1
DATE=`date +%s`      
echo "$DATE: Starting GT4 container using config file ${CONFIG_FILE} on ${MACH_ID} and port ${FALKON_PORT}..." >> ${LOG_FILE}
echo "$DATE: Starting GT4 container using config file ${CONFIG_FILE} on ${MACH_ID} and port ${FALKON_PORT}..."
